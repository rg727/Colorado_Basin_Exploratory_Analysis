...
MonthlyQ = readFiles('ym2015x.xbm', 16, 94)
LastNodeFractions = np.zeros([2013-1982,61,12])
for i in range(2013-1982):
    LastNodeFractions[i,0,:] = MonthlyQ[74+i,:,-1]/np.sum(MonthlyQ[74+i,:,-1])

# read in daily flows at last node
LastNodeQ = pd.read_csv('last_node.csv')
...
for shift in shifts:
    ...
    for i in range(len(years)-1):
        ...
        natural_cdf = np.cumsum(MonthlyQ[74+i,:,-1])
...
LastNodeFractions_preRecord = np.zeros([1983-1909,61,12])
for i in range(1983-1909):
    d = np.zeros(2013-1982)
    for j in range(2013-1982):
        for k in range(12):
            d[j] += (MonthlyQ[i,k,-1]/np.sum(MonthlyQ[i,:,-1]) - MonthlyQ[j+16,k,-1]/np.sum(MonthlyQ[j+16,:,-1]))**2
