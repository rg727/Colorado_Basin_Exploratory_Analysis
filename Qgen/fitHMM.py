...
def fitHMM(TransformedQ):
    ...
    return X,Z,hidden_states, mus, sigmas, P
...
def plotDistribution(TransformedQ, mus, sigmas, P):
    ...
    fig.savefig('MixedGaussianFit.png',dpi=200)
    fig.clf()
...
AnnualQ = np.array(pd.DataFrame.from_csv('AnnualQ_Gunnison.csv'))

logQ = np.log(AnnualQ[:,-1])
X,Z,hidden_states, mus, sigmas, P = fitHMM(logQ)
