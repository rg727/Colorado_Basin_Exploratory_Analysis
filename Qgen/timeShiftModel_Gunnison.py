...
MonthlyQ = readFiles('D:/Documents/gm2015_StateMod_modified/gm2015_StateMod_modified/StateMod/gm2015x.xbm', 16, 139)
LastNodeFractions = np.zeros([2013-1916,61,12])
for i in range(2013-1916):
    LastNodeFractions[i,0,:] = MonthlyQ[8+i,:,-1]/np.sum(MonthlyQ[8+i,:,-1])

# read in daily flows at last node
LastNodeQ = pd.read_csv('D:/Documents/UCRB_analysis-master/Qgen/last_node_gunnison.csv')
...
for i in range(len(years)-1):
    plt.plot(MonthlyQ[74+i,:,-1])
    for shift in shifts:
        plt.plot(np.sum(MonthlyQ[74+i,:,-1]) * LastNodeFractions[i,shift,:], c=cmap(shift/30))
...
LastNodeFractions_preRecord = np.zeros([1917-1909,61,12])
for i in range(1917-1909):
    d = np.zeros(2013-1916)
    for j in range(2013-1917):
        for k in range(12):
            d[j] += (MonthlyQ[i,k,-1]/np.sum(MonthlyQ[i,:,-1]) - MonthlyQ[j+8,k,-1]/np.sum(MonthlyQ[j+8,:,-1]))**2
...
np.save('LastNodeFractions_Gunnison',LastNodeFractions)