...
MonthlyQ_all = readFiles('D:/Documents/UCRB_analysis-master/Data/'+name+'/'+abbrev+'2015_StateMod_modified/StateMod/'+abbrev+'2015x.xbm', 16, nSites)
...
MonthlyQ_h = np.array(pd.read_csv('MonthlyQ_'+name+'.csv'))
AnnualQ_h = np.array(pd.read_csv('AnnualQ_'+name+'.csv'))
...
AnnualIWR_h = np.loadtxt('AnnualIWR_'+name+'.csv',delimiter=',')
MonthlyIWR_h = np.loadtxt('MonthlyIWR_'+name+'.csv',delimiter=',')
...
for i in range(start, stop):
    for realization in range(10):
       # create matrices to store nYears of synthetic (_s), real-space (AnnualQ_s), 
        ...
        writeNewFiles('D:/Documents/UCRB_analysis-master/Data/'+name+'/'+abbrev+'2015_StateMod_modified/StateMod/'+abbrev+'2015x.xbm', startXBM, i, realization+1, MonthlyQ_s, '')
        
        # write new irrigation demands to file for LHsample i
        writeNewFiles('D:/Documents/UCRB_analysis-master/Data/'+name+'/'+abbrev+'2015_StateMod_modified/StateMod/'+abbrev+'2015B.iwr', startIWR, i, realization+1, MonthlyIWR_s, 'a') # a for all uncertainties
        
        # write new irrigation demands to file for LHsample i
        writeNewFiles('D:/Documents/UCRB_analysis-master/Data/'+name+'/'+abbrev+'2015_StateMod_modified/StateMod/'+abbrev+'2015B.iwr', startIWR, i, realization+1, MonthlyIWR_s_flowOnly, 'f') # f for flow uncertainties only
